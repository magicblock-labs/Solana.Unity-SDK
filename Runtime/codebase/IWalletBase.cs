...
namespace Solana.Unity.SDK
{
    ...
    public abstract class WalletBase : IWalletBase
    {
        ...
        public async Task<bool> CanLogin()
        {
            try
            {
                // Attempt to login without providing a password, if successful, return true
                await _Login();
                return true;
            }
            catch (Exception)
            {
                // If login fails (due to absence of supported wallet or any other reason), return false
                return false;
            }
        }
    }
